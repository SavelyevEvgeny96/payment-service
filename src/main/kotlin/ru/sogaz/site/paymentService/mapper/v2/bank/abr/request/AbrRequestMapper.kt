...
abstract class AbrRequestMapper {
    ...
    companion object {
        ...
        private const val HPP_COF_CAPTURE_PURPOSES_RECURRING = "Recurring"
        ...
        fun mapHppCofCapturePurposes(saveCard: Boolean): String? =
            HPP_COF_CAPTURE_PURPOSES_RECURRING.takeIf { saveCard }
    }
    ...
    fun toCardRequest(
        ...
        toAbrRequest(cardPayOperationRequest, TypeRidEnum.WITH_3DS, redirectUrl)
    ...
    @Mapping(target = "order.hppRedirectUrl", source = "redirectUrl")
    @Mapping(target = "order.adviceIfaceAddress", source = "redirectUrl")
    @Mapping(target = "order.hppCofCapturePurposes", source = "request.saveCard", qualifiedByName = ["mapHppCofCapturePurposes"])
    @Mapping(target = "order.description", source = "request.description")
    @Mapping(target = "order.language", constant = "RU")
    ...
    @Mapping(target = "order.descriptionHtml", source = "request.description")
    @Mapping(target = "order.expTime", source = "typeRid", qualifiedByName = ["mapExpTime"])
    @Mapping(target = "order.hppCofCapturePurposes", ignore = true)
    protected abstract fun toAbrRequest(
