 ...
 */
@Tag(name = "Payment API", description = "API для обработки платежей")
@RestController
...
class PaymentController(
     ...
     * @param requestWrapper Данные запроса на оплату
            ...
            ApiResponse(
                ...
                content = [
                    Content(
                        schema =
                            Schema(
                                example = "{\"status\": \"error\", \"code\": -1101500401, \"messageError\": \"Ваш запрос не авторизован\"}",
                            ),
                    ),
                ],
            ),
                ...
                content = [
                    Content(
                        schema =
                            Schema(
                                example =
                                    "{\"status\": \"error\", \"code\": -1101500403, \"messageError\": " +
                                        "\"Вам запрещен доступ к запрашиваемому ресурсу\"}",
                            ),
                    ),
                ],
            ),
                ...
                content = [
                    Content(
                        schema =
                            Schema(
                                example =
                                    "{\"status\": \"error\", \"code\": -1101500422, \"messageError\": " +
                                        "\"Не все обязательные данные указаны корректно\"}",
                            ),
                    ),
                ],
            ),
    ...
    @PostMapping("/create")
    fun createOrder(
        @Parameter(description = "Идентификатор трассировки", required = true)
        @RequestHeader("TraceId") traceId: String,
        @RequestBody(description = "Данные для создания платежа", required = true)
